        ...
        return (
            <>
                ...
                <Grid container className={classes.layout} spacing={8}>
                    <Grid item xs={12}>
                        <Typography component="h1" variant="h2">{this.props.project.id}</Typography>
                    </Grid>
                    <Grid item xs={12}>
                        <ProjectSummary
                            totalDownloads={this.props.project.total_downloads}
                            totalDownloads30Days={Object.values(this.props.project.downloads).reduce((carry, x) => carry + x)}
                            totalDownloads7Days={Object.values(this.props.project.downloads).slice(0, 7).reduce((carry, x) => carry + x)}

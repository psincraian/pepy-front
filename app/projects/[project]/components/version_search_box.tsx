...
function VersionSearchBox({
  downloads,
  onChange,
  selectedVersions,
  versions,
}: VersionSearchBoxProps) {
  const versionDownloadsCache = useMemo(() => {
    return versions.reduce(
      (cache: Record<string, number>, version: OptionType) => {
        cache[version.value] = Object.values(downloads).reduce(
          (total: number, curr: Record<string, number>) =>
            total + (curr[version.value] || 0),
          0,
        );
        return cache;
      },
      ...
      <Autocomplete
        ...
        renderInput={(params) => (
          <TextField
            {...params}
            variant="outlined"
            label="Select versions. Use * to perform queries"
          />
        )}
        ...
        onChange={(event, value: readonly (string | OptionType)[]) =>
          onChange(value.map((x) => (typeof x === "string" ? x : x.value)))
        }

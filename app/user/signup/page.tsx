...
export default function Home() {
    ...
    if (submissionStatus.status === SignupState.STARTED) {
        content = <SignupForm
            onSuccess={(username) => setSubmissionStatus({status: SignupState.CODE_VERIFICATION, username: username})}/>
    } else if (submissionStatus.status === SignupState.CODE_VERIFICATION) {
        content = <ConfirmSignupForm username={submissionStatus.username} onSuccess={() => setSubmissionStatus({
            ...submissionStatus,
        ...
        content = (
            ...
            >
                ...
                <Grid item xs={12} sm={8}>
                    <Button variant="contained" onClick={() => router.push("/user/login")}>Login</Button>
                </Grid>
            </Grid>
        )
    }
